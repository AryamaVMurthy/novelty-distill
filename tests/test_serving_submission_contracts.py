...
def test_turing_submit_stages_in_login_tmp_instead_of_quota_limited_home() -> None:
    script = Path("scripts/turing_submit.sh").read_text(encoding="utf-8")

    assert 'remote_user="$(ssh turing' in script
    assert 'remote_stage="/tmp/novelty-distill-submit-${remote_user}/' in script
...
def test_teacher_clustering_supports_gpu_partitions_and_strict_merge() -> None:
    ...
    assert 'samples_per_prompt="${SAMPLES_PER_PROMPT:-8}"' in cluster
    assert '--samples-per-prompt "${samples_per_prompt}"' in cluster
