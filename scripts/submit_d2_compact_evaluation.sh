...
printf '{"study":"tomato1k-compact-k4-d2-extension","generation_array":"%s","generation_gate":"%s","score_array":"%s","score_gate":"%s","evaluation_job":"%s","analysis_job":"%s"}\n' \
  "${generation_array}" "${generation_gate}" "${score_array}" "${score_gate}" \
