#!/bin/bash
# Submit missing random-1 SeqKD generation, judging, corrected evaluation, and analysis.
...
eval_id="B2a-tomato1k-seed17-temporal-k4"
evaluation_output="${eval_id}-corrected-v2"
...
adapter_path="checkpoints/B2a-tomato1k-seed17/final"
artifact_identity="sha256:812cb0cbb2b8bc6efbd5163aeb5c54b2c74b0cd4f400ba8b703cd7a62b2aa7f5"
...
evaluation_job="$(
  submit_job slurm/evaluate_student.sbatch \
    ...
    --export="ALL,TEACHER_SCORE_ID=${teacher_id},STUDENT_SCORE_ID=${eval_id},SCORE_NAMESPACE=evaluation-scores,TARGET_NAME=teacher-targets-tomato1k-v1.json,OUTPUT_NAME=${evaluation_output},TEACHER_SAMPLES_PER_PROMPT=4,STUDENT_SAMPLES_PER_PROMPT=4,TEACHER_SOURCE_SAMPLES_PER_PROMPT=16,STUDENT_SOURCE_SAMPLES_PER_PROMPT=4"
)"
analysis_job="$(
  submit_job slurm/analyze_b2a_compact_extension.sbatch \
    --nodelist="${target_node}" \
    --dependency="afterok:${evaluation_job}"
)"

printf '{"study":"tomato1k-compact-k4-b2a-extension","target_node":"%s","generation_array":"%s","generation_gate":"%s","score_array":"%s","score_gate":"%s","evaluation_job":"%s","analysis_job":"%s"}\n' \
  "${target_node}" "${generation_array}" "${generation_gate}" "${score_array}" \
  "${score_gate}" "${evaluation_job}" "${analysis_job}"
